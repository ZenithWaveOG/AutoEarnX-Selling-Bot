...
ADMIN_IDS = [8537079657]  # Replace with your Telegram user ID
...
def get_min_quantity(coupon_type):
    prices = supabase.table('prices').select('min_quantity').eq('coupon_type', coupon_type).execute()
    if prices.data and prices.data[0].get('min_quantity') is not None:
        return prices.data[0]['min_quantity']
    return 1
...
# --- Terms callback with coupon question ---
...
# --- Quantity selection with min check ---
...
# --- Process quantity, create order, show invoice ---
...
async def admin_accept_decline(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "accept":
        ...
        if discount_code:
            supabase.table('discount_codes').update({'used': True}).eq('code', discount_code).execute()
        context.user_data.pop('discount_code', None)
        context.user_data.pop('discount_value', None)
...
telegram_app.add_handler(coupon_conv_handler)

# Callback query handlers
telegram_app.add_handler(CallbackQueryHandler(terms_callback, pattern="^(agree|decline)_terms$"))
telegram_app.add_handler(CallbackQueryHandler(have_coupon_callback, pattern="^have_coupon_"))
...
# Initialize application on background loop
async def init_app():
